		...
		if message.text.isdigit():
			...
			if len(result) == 0:
				...
				if id == 0:
					...
					connection.commit()
					await message.answer('Пользователь добавлен в ЧС', reply_markup=kb.adm)
					await state.finish()
					await bot.send_message(message.text, 'Тебя добавили в ЧС')
				else:
					await message.answer('Данный пользователь уже в ЧС', reply_markup=kb.adm)
					await state.finish()
		else:
